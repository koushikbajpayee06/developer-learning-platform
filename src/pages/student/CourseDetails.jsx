      ...
      <div className='flex md:flex-row flex-col-reverse gap-10 relative items-start justify-between md:px-36 px-8 md:pt-30 pt-20 text-left'>
        ...
        <div className='max-w-xl z-10 text-gray-500'>
          <h1 className='md:text-course-details-heading-large text-course-details-heading-small 
            font-semibold text-gray-800'>{courseData?.courseTitle}
          </h1>
            <p className='pt-4 rich-text' dangerouslySetInnerHTML={{__html: courseData?.courseDescription.slice(0, 200) + '...'}}></p>
          ...
          {/* review  and ratings */}
           <div className='flex items-center space-x-2 pt-3 pb-1 text-sm'>
            ...
            <p className='text-sm'>Course by <span className='text-blue-600 underline'>NamasteDev</span></p>
                    ...
                    courseData?.courseContent.map((chapter, index)=>(
                      <div key={index} className='border border-gray-300 bg-white mb-2 rounded'>
                        <div className='flex items-center justify-between px-4 py-3 cursor-pointer select-none'
                          ...
                          <p className='text-sm md:text-default'>{chapter.chapterContent.length} lectures - {calculateCourseTime(chapter)}</p>
                        </div>
